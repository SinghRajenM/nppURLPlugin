...
{
	const std::string& encode(const std::string& aString)
	{
	...
	std::string encode(const std::pair<std::string, std::string>& pair)
	{
		std::string aEscapedString = "";
		aEscapedString += encode(pair.first);
		aEscapedString += "=";
		aEscapedString += encode(pair.second);
		aEscapedString += "&";
...
public:
	...
	{
		return _url.empty() ? _url : encode(_url);
	}
}; // class Encode
